...
def fuzzy_match_track(
    local_filename: str, track_titles: List[str], threshold: int = 75
) -> Tuple[str, int]:
    """Match local filename to Discogs track titles using fuzzy logic."""
    local_clean = (
        os.path.splitext(local_filename)[0].replace("-", " ").replace("_", " ").lower()
    )
    ...
    for title in track_titles:
        ...
        word_match = (
            sum(1 for w in words if w in title_clean) / max(len(words), 1) * 100
        )
...
def embed_artwork(file_path, artwork_bytes: bytes, format_name: str):
    ...
    try:
        ...
        if format_name == "MP3":
            ...
            audio["metadata_block_picture"] = mutagen.ogg.vorbis.encode_picture(
                artwork_bytes
            )
...
def write_metadata(
    file_path: str, tags: dict, artwork_bytes: bytes = None, format_name: str = ""
):
    ...
    try:
        ...
        for key, val in tags.items():
            if hasattr(audio, "tags"):
                if key == "title":
                    audio.tags.add(TIT2(encoding=3, text=str(val)))
                elif key == "artist":
                    audio.tags.add(TPE1(encoding=3, text=str(val)))
                elif key == "album":
                    audio.tags.add(TALB(encoding=3, text=str(val)))
                elif key == "tracknumber":
                    audio.tags.add(TRCK(encoding=3, text=str(val)))
                elif key == "date":
                    audio.tags.add(TYER(encoding=3, text=str(val)))
                elif key == "genre":
                    audio.tags.add(TCON(encoding=3, text=str(val)))
